...
# against this project's current working tree. Does NOT reset or build/verify on its own - run
# reset-to-starting-point.sh first, and premigration-build-java21.sh/postmigration-build-java25.sh
# around this to get the full demo sequence (see full-demo.sh for all of it chained together).
set -euo pipefail
...
RECIPE_VERSION=$("$SCRIPT_DIR/build-java25-recipes.sh")
